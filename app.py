...
def conectar_db():
    ...
    # Popular dados fictícios se estiver vazio
...
# Inicialização com limpeza forçada em caso de erro estrutural
try:
    conn = conectar_db()
except Exception as e:
    st.warning("A reestruturar base de dados antiga...")
...
if not st.session_state['autenticado']:
    ...
    if escolha == "📊 Dashboard & KPIs":
        ...
        try:
            ...
            st.info("Sem dados de veículos para exibir gráficos.")
        ...
        with tab_veic:
            with st.form("form_cadastro_veiculo", clear_on_submit=True):
                ...
                doc_veiculo = st.text_area("Informações do Documento")
        ...
        with tab_mot:
            with st.form("form_cadastro_motorista", clear_on_submit=True):
                ...
                aceitou_termo = st.checkbox("Motorista aceita o termo de uso")
        ...
        if df_veiculos.empty:
            ...
            with st.form("form_km_diario", clear_on_submit=True):
                ...
                if st.form_submit_button("Atualizar Quilometragem"):
                    cursor = conn.cursor()
        ...
        if df_veiculos_global.empty:
            ...
            with st.form("form_checklist", clear_on_submit=True):
                ...
                tipo = st.selectbox("Tipo de Movimentação", ["Entrada de Oficina", "Saída de Oficina", "Novo Contrato", "Devolução"])
                ...
                if st.form_submit_button("Transmitir Checklist"):
                    cursor = conn.cursor()
            ...
            with tab2:
                ...
                if df_os.empty:
                    ...
                    for idx, row in df_os.iterrows():
                        ...
                        col1.write(f"**OS #{row['id']}** - {row['placa']} | Custo: R$ {row['custo']}")
